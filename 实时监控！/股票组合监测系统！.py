...
def calculate_stock_position_limit(cumulative_increase):
    """计算个股仓位限制"""
...
def calculate_stock_reduction(stock_position):
    """计算个股减仓比例"""
    cumulative_increase = stock_position.get('cumulative_increase', 0)
    drawdown = stock_position.get('drawdown', 0)
    position_ratio = stock_position.get('position_ratio', 0)
    ...
    if cumulative_increase > 0:
        if drawdown > -15 and drawdown <= -4.5:
            ...
            return round(reduction_ratio, 1), f"减仓提示：回撤{drawdown:.1f}%，累计涨幅{cumulative_increase:.1f}%，建议减仓比例{reduction_ratio:.1f}%", False
        elif drawdown <= -15:
            return round(position_ratio, 1), f"清仓提示：回撤已达{abs(drawdown):.1f}%，超过15%红线，建议清仓", True
    else:
        loss = abs(cumulative_increase)
        if position_ratio > 6:
            return round(position_ratio - 6, 1), f"仓位超限：处于亏损状态，仓位需控制在6%以内，当前{position_ratio:.1f}%，需减仓{position_ratio - 6:.1f}%", False
        reduction_ratio = loss * 0.15 * position_ratio
        reduction_ratio = min(reduction_ratio, position_ratio)
        if loss >= 4.5:
            return round(position_ratio, 1), f"清仓提示：亏损{loss:.1f}%，已达止损线，建议清仓", True
        return round(reduction_ratio, 1), f"减仓提示：亏损{loss:.1f}%，建议减仓比例{reduction_ratio:.1f}%", False
    return 0, "持有", False
...
def update_summary():
    ...
    for pos in portfolio.get('positions', []):
        ...
        position_details.append({
            ...
            'positionRatio': 0,
            'lowestPrice': pos.get('lowestPrice', 0),
            'lowestDate': pos.get('lowestDate', ''),
            'cumulativeIncrease': 0,
            'drawdown': 0,
            'reasonablePositionRatio': 20,
            'reductionRatio': 0,
            'reductionMessage': '',
            'positionLimitStatus': '正常'
        })
    ...
    for detail in position_details:
        ...
        if HAS_IFIND:
            try:
                ...
                # 获取过去1个季度的历史数据
                result = THS_HQ(code, 'close', '', quarter_start, current_date, 'format:dataframe')
                if result and result.errorcode == 0 and result.data is not None and len(result.data) > 0:
                    df = result.data
                    if 'close' in df.columns:
                        quarter_low = df['close'].min()
                        if quarter_low > 0:
                            cumulative_increase = (current_price - quarter_low) / quarter_low * 100
                        else:
                ...
                print(f"获取季度低点失败 {code}: {e}")
                cumulative_increase = 0
        else:
            # 如果没有同花顺接口，使用原来的逻辑
            if detail['lowestPrice'] > 0:
                cumulative_increase = (buy_price - detail['lowestPrice']) / detail['lowestPrice'] * 100 if detail['lowestPrice'] > 0 else 0
            else:
                cumulative_increase = 0
        
        # 计算最大累计回撤（买入时点到最新时点期间，价格从最高点到现价的累计回调幅度）
        if HAS_IFIND:
            try:
                THS_iFinDLogin(IFIND_USER, IFIND_PASS)
                from datetime import datetime, timedelta
                current_date = datetime.now().strftime('%Y-%m-%d')
                buy_date = detail.get('buyDate', current_date)
                
                # 获取买入日期到当前日期的历史数据
                result = THS_HQ(code, 'close', '', buy_date, current_date, 'format:dataframe')
                if result and result.errorcode == 0 and result.data is not None and len(result.data) > 0:
                    df = result.data
                ...
                print(f"获取最大回撤数据失败 {code}: {e}")
                # 如果获取失败，使用原来的逻辑作为备用
                drawdown = (current_price - buy_price) / buy_price * 100 if buy_price > 0 else 0
        else:
            # 如果没有同花顺接口，使用原来的逻辑
            drawdown = (current_price - buy_price) / buy_price * 100 if buy_price > 0 else 0
        
        detail['cumulativeIncrease'] = round(cumulative_increase, 1)
        detail['drawdown'] = round(drawdown, 1)
        
        limit, status, msg = calculate_stock_position_limit(cumulative_increase)
        detail['reasonablePositionRatio'] = limit
        detail['positionLimitStatus'] = status
        reduction, reduction_msg, should_sell_all = calculate_stock_reduction({
            'cumulative_increase': cumulative_increase,
            'drawdown': drawdown,
            'position_ratio': detail['positionRatio']
        })
        detail['reductionRatio'] = reduction
        detail['reductionMessage'] = reduction_msg
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        td:nth-child(8), /* 仓位限制 */
        td:nth-child(9), /* 变动金额 */
        td:nth-child(10), /* 累计获利/亏损幅度 */
        td:nth-child(11), /* 净值贡献占比 */
        td:nth-child(12), /* 底部累计上涨 */
        td:nth-child(13), /* 最大回撤 */
        td:nth-child(14), /* 减仓比例 */
        td:nth-child(15) /* 合理持仓 */ {
            text-align: center;
        }
        ...
        /* 个股仓位控制样式 */
        .position-limit-tag { display: inline-block; padding: 2px 8px; border-radius: 3px; font-size: 11px; font-weight: bold; }
        .position-limit-tag.正常 { background: rgba(0,255,136,0.2); color: #00ff88; }
        .position-limit-tag.止盈 { background: rgba(255,193,7,0.2); color: #ffc107; }
        .position-limit-tag.警告 { background: rgba(255,71,87,0.2); color: #ff4757; }
        .position-alert { padding: 8px 12px; border-radius: 6px; margin-bottom: 8px; font-size: 12px; }
        .position-alert.warning { background: rgba(255,193,7,0.15); border: 1px solid #ffc107; color: #ffc107; }
        .position-alert.danger { background: rgba(255,71,87,0.15); border: 1px solid #ff4757; color: #ff4757; }
    </style>
</head>
<body>
    <div class="container">
            ...
            <div class="card" id="addStockCard" style="display:none;">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>买入占比(%)</label>
                        <input type="number" id="buyRatio" placeholder="如: 10">
                        <div id="positionLimitInfo" style="font-size:11px;color:#8b9dc3;margin-top:4px;"></div>
                    </div>
    ...
    <script>
        ...
        function fetchPrice() {
            ...
            document.getElementById('positionLimitInfo').innerHTML = '正在计算仓位限制...';
            ...
            fetch(`/api/price/${fullCode}`)
                ...
                .then(data => {
                    if(data.price && data.price > 0) {
                        ...
                        // 获取仓位限制信息
                        fetch(`/api/position-limit/${fullCode}`)
                            .then(response => response.json())
                            .then(limitData => {
                                let limitHtml = '';
                                if (limitData.positionStatus === '止盈') {
                                    limitHtml = `<span style="color: #ffc107; font-weight: bold;">${limitData.message}</span>`;
                                } else {
                                    limitHtml = `<span style="color: #00ff88;">${limitData.message}</span>`;
                                }
                                document.getElementById('positionLimitInfo').innerHTML = limitHtml;
                            })
                            .catch(error => {
                                console.error('获取仓位限制失败:', error);
                                document.getElementById('positionLimitInfo').innerHTML = '';
                            });
                    } else {
        ...
        function addStock() {
            ...
            fetch(`/api/position-limit/${fullCode}`)
                .then(response => response.json())
                .then(limitData => {
                    if (ratio > limitData.positionLimit) {
                        return showAlert('buildAlert', `买入占比${ratio}%超过仓位限制${limitData.positionLimit}%，请降低买入比例`, 'warning');
                    }
                    
                    // 检查仓位控制
                    return fetch('/api/summary').then(r=>r.json()).then(data => {
                ...
                .catch(error => {
                    ...
                    fetch('/api/summary').then(r=>r.json()).then(data => {
                        ...
                        // 继续执行建仓逻辑...
                        // 这里可以复用上面的建仓代码，但为了简化，我们只显示错误
                        showAlert('buildAlert', '获取仓位限制信息失败，请重试', 'error');
                    });
                });
        }
        ...
        function renderPositions() {
            console.log('开始渲染持仓表格...');
            fetch('/api/summary').then(r=>r.json()).then(data => {
                console.log('获取到汇总数据:', data);
                ...
                let html = '<table><thead><tr><th>代码</th><th>名称</th><th>股数</th><th>成本价</th><th>现价</th><th>市值(¥)</th><th>持仓比例</th><th>仓位限制</th><th>变动金额(¥)</th><th>累计获利/亏损幅度</th><th>净值贡献占比</th><th>底部累计上涨</th><th>最大回撤</th><th>减仓比例</th><th>合理持仓</th></tr></thead><tbody>';
                ...
                data.positions.forEach(p => {
                    ...
                    // 累计获利/亏损幅度
                    const profitLossClass = p.changePct >= 0 ? 'positive' : 'negative';
                    const profitLossDisplay = p.changePct >= 0 ? '<span class="positive">+' + p.changePct.toFixed(1) + '%</span>' : '<span class="negative">' + p.changePct.toFixed(1) + '%</span>';
                    
                    // 净值贡献占比
                    const contribClass = p.contribution >= 0 ? 'positive' : 'negative';
                    const contribDisplay = p.contribution >= 0 ? '<span class="positive">+' + p.contribution.toFixed(1) + '%</span>' : '<span class="negative">' + p.contribution.toFixed(1) + '%</span>';
                    
                    // 底部累计上涨
                    const cumulativeDisplay = p.cumulativeIncrease >= 0 ? '<span class="positive">+' + p.cumulativeIncrease.toFixed(1) + '%</span>' : '<span class="negative">' + p.cumulativeIncrease.toFixed(1) + '%</span>';
                    
                    // 最大回撤
                    const drawdownDisplay = p.drawdown >= 0 ? '<span class="positive">' + p.drawdown.toFixed(1) + '%</span>' : '<span class="negative">' + p.drawdown.toFixed(1) + '%</span>';
                    
                    // 仓位限制
                    const positionLimit = p.reasonablePositionRatio || 20;
                    const limitStatus = p.positionLimitStatus || '正常';
                    let limitDisplay;
                    if (limitStatus === '止盈') {
                        limitDisplay = '<span style="color: #ffc107; font-weight: bold;">' + positionLimit + '%</span>';
                    } else {
                        limitDisplay = '<span style="color: #00ff88;">' + positionLimit + '%</span>';
                    }
                    
                    // 合理持仓 = min(持仓比例，仓位限制) - 减仓比例
                    const maxAllowedRatio = Math.min(p.positionRatio, positionLimit);
                    const reasonableRatio = Math.max(0, maxAllowedRatio - (p.reductionRatio || 0));
                    const isUnreasonable = p.positionRatio > (p.reasonablePositionRatio || 20);
                    const reasonableDisplay = isUnreasonable ? '<span style="color: #ff4757; font-weight: bold;">' + reasonableRatio.toFixed(1) + '%</span>' : reasonableRatio.toFixed(1) + '%';
                    
                    // 调试信息
                    console.log(`股票 ${p.code}: 持仓比例=${p.positionRatio.toFixed(1)}%, 仓位限制=${positionLimit}%, 最大允许=${maxAllowedRatio.toFixed(1)}%, 减仓比例=${(p.reductionRatio || 0).toFixed(1)}%, 合理持仓=${reasonableRatio.toFixed(1)}%`);
                    
                    // 减仓比例
                    let reductionDisplay = '-';
                    if(p.reductionRatio > 0) {
                        reductionDisplay = '<span class="negative">' + p.reductionRatio.toFixed(1) + '%</span>';
                    } else {
                        reductionDisplay = '<span class="positive">无</span>';
                    }
                    
                    html += '<tr><td>' + p.code + '</td><td>' + p.name + '</td><td>' + p.shares.toLocaleString() + '</td><td>' + costDisplay + '</td><td>' + priceDisplay + '</td><td style="text-align:right;">¥' + formatNumber(Math.round(p.marketValue)) + '</td><td>' + p.positionRatio.toFixed(1) + '%</td><td>' + limitDisplay + '</td><td class="' + changeClass + '" style="text-align:right;">' + (p.change>=0?'+':'') + '¥' + formatNumber(Math.round(p.change)) + '</td><td class="' + profitLossClass + '">' + profitLossDisplay + '</td><td class="' + contribClass + '">' + contribDisplay + '</td><td>' + cumulativeDisplay + '</td><td>' + drawdownDisplay + '</td><td>' + reductionDisplay + '</td><td>' + reasonableDisplay + '</td></tr>';
                });
                html += '</tbody></table>';
                ...
                let alerts = '';
                data.positions.forEach(p => {
                    if(p.reductionMessage && p.reductionRatio > 0) {
                        const alertClass = p.reductionRatio >= p.positionRatio ? 'danger' : 'warning';
                        alerts += '<div class="position-alert ' + alertClass + '">' + p.reductionMessage + '</div>';
                    }
                    const reasonableLimit = p.reasonablePositionRatio || 20;
                    if(p.positionRatio > reasonableLimit) {
                        alerts += '<div class="position-alert danger">⚠️ ' + p.name + '持仓比例' + p.positionRatio.toFixed(1) + '%超过合理持仓限制' + reasonableLimit + '%，请止盈或减仓</div>';
                    }
                });
                
                container.innerHTML = alerts + html;
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('当前portfolio数据:', portfolio);
...
@app.route('/api/position-limit/<code>')
def get_position_limit(code):
    """获取股票仓位限制信息"""
    try:
        if HAS_IFIND:
            THS_iFinDLogin(IFIND_USER, IFIND_PASS)
            ...
            current_price = 0
            try:
                result = THS_HQ(code, 'close', '', current_date, current_date, 'format:dataframe')
                if result and result.errorcode == 0 and result.data is not None and len(result.data) > 0:
                    df = result.data
                    if 'close' in df.columns and len(df) > 0:
            ...
            if result and result.errorcode == 0 and result.data is not None and len(result.data) > 0:
                ...
                if 'close' in df.columns:
                    quarter_low = df['close'].min()
                    if quarter_low > 0 and current_price > 0:
                        cumulative_increase = (current_price - quarter_low) / quarter_low * 100
                        ...
                        if cumulative_increase >= 65 and cumulative_increase < 100:
                            limit = 10
                            status = "止盈"
                            message = f"由于底部上涨幅度{cumulative_increase:.1f}%超过65%，仓位限制至10%"
                        elif cumulative_increase >= 100:
                            limit = 5
                            status = "止盈"
                            message = f"由于底部上涨幅度{cumulative_increase:.1f}%超过100%，仓位限制至5%"
                        else:
                            limit = 20
                            status = "正常"
                            message = "仓位限制20%以内"
                        ...
                        return jsonify({
                            'cumulativeIncrease': round(cumulative_increase, 1),
                            'positionLimit': limit,
                            'positionStatus': status,
                            'message': message,
                            'quarterLow': round(quarter_low, 2),
